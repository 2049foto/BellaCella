/* BELLA CELLA — nguồn sự thật duy nhất cho nội dung sản phẩm.
   Mọi chữ trong file này trích nguyên văn từ Catalogue_BELLA_CELLA_VN.pdf.
   Không sửa nội dung mà không đối chiếu catalogue.
   COMMERCE_ENABLED=false -> nút hành động là "Nhận tư vấn".
   Bật true -> "Thêm vào giỏ" + mở giỏ hàng. */
...
export type Step = { n: number; label: string; slug: string };
...
/* ---------------------------------------------------------------
   ...
   --------------------------------------------------------------- */
export type KnowledgeSource = { t: string; u: string };
